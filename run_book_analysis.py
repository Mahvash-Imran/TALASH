"""
run_book_analysis.py  –  CLI Entry Point for Module 5
=====================================================

Usage
-----
  # Full run with LLM summaries:
  python run_book_analysis.py

  # Skip LLM (rule-based summaries only, instant, no API cost):
  python run_book_analysis.py --skip-llm

  # Custom paths:
  python run_book_analysis.py \
      --books-csv      data/extracted/books.csv \
      --candidates-csv data/extracted/candidates.csv \
      --output-dir     data/analysis

  # Use a specific model:
  python run_book_analysis.py --model meta-llama/llama-4-scout-17b-16e-instruct
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("data/logs/book_analysis.log", mode="a", encoding="utf-8"),
    ],
)
logger = logging.getLogger("talash.book.main")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="TALASH Module 5 – Books Authored / Co-Authored Analysis"
    )
    parser.add_argument(
        "--books-csv",
        default=os.environ.get("BOOKS_CSV", "data/extracted/books.csv"),
        help="Path to books.csv from Module 1",
    )
    parser.add_argument(
        "--candidates-csv",
    ...
    parser.add_argument(
        ...
        help="Directory for Module 5 output files",
    )
    parser.add_argument(
        "--skip-llm",
        action="store_true",
